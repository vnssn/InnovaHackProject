...
class AnalyticsRepository:
    ...
    async def get_dashboard(self, user_id: uuid.UUID) -> dict:
        ...
        ).where(Transaction.user_id == user_id).group_by(text("month")).order_by(text("month desc")).limit(2).subquery()

        result = await self.db.execute(select(
            func.coalesce(func.sum(Transaction.amount), 0),
        ).where(
            Transaction.user_id == user_id,
            func.date_trunc("month", Transaction.transaction_date) == func.date_trunc("month", func.now()),
        ))
        monthly_spending = float(result.scalar() or 0)

        today_result = await self.db.execute(select(
            func.coalesce(func.sum(Transaction.amount), 0),
        ).where(
            Transaction.user_id == user_id,
            func.date_trunc("day", Transaction.transaction_date) == func.date_trunc("day", func.now()),
        ))
        ...
        monthly_averages = await self.db.execute(
            select(func.avg(current_month.c.total))
        )
        avg_monthly = float(monthly_averages.scalar() or monthly_spending)

        spending_change = round(((monthly_spending - avg_monthly) / avg_monthly * 100), 1) if avg_monthly > 0 else 0
    ...
    async def get_trends(self, user_id: uuid.UUID, period: str = "6m", category_id: uuid.UUID | None = None) -> list[dict]:
        ...
        query = select(
            ...
            Transaction.user_id == user_id,
            Transaction.transaction_date >= func.now() - text(f"interval '{months} months'"),
        )
        if category_id:
