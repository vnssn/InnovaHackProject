    ...
    <header className="sticky top-0 h-16 w-full bg-surface/80 backdrop-blur-xl border-b border-outline-variant/30 z-40 px-md lg:px-xl flex items-center justify-between">
      ...
      <div className="flex items-center gap-md">
        <div className="relative">
          <div className="relative p-base hover:bg-surface-container rounded-full cursor-pointer transition-colors group" onClick={() => setShowNotifications(!showNotifications)}>
          ...
          {showNotifications && (
            <div className="absolute right-0 mt-2 w-80 bg-surface-container border border-outline-variant/30 rounded-2xl shadow-2xl backdrop-blur-xl p-md z-50">
              <h3 className="font-label-md text-label-md text-on-surface mb-sm">Notifications</h3>
              {notificationsData?.items?.length > 0 ? (
                <div className="flex flex-col gap-sm">
                  {notificationsData.items.slice(0, 5).map((n: any) => (
                    <div key={n.id} className="p-sm bg-surface-container-highest/50 rounded-lg">
        ...
        <div className="relative">
          <div className="flex items-center gap-sm pl-md border-l border-outline-variant/50 cursor-pointer group" onClick={() => setShowUserMenu(!showUserMenu)}>
